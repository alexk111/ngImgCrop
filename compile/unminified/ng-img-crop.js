...
crop.factory('cropAreaRectangle', ['cropArea', function (CropArea) {
    ...
    CropAreaRectangle.prototype._calcRectangleCorners = function () {
        var size = this.getSize();  //小方块的大小
        var se = this.getSouthEastBound();  //中间亮层的大小
        return [
            [size.x, size.y], //northwest
            [se.x, size.y], //northeast
            [size.x, se.y], //southwest
            [se.x, se.y], //southeast
            [(size.x + se.x) / 2, size.y],//topcenter
            [(size.x + se.x) / 2, se.y],//bottomcenter
            [size.x, (size.y + se.y) / 2],//leftcenter
            [se.x, (size.y + se.y) / 2],//rightcenter
    ...
    CropAreaRectangle.prototype._clampPoint = function(x, y) {
    ...
    CropAreaRectangle.prototype.processMouseMove = function (mouseCurX, mouseCurY) {
        ...
        if (this._areaIsDragging) {
            ...
            var s = this.getSize();   // x y w h   xy为左上角  wh为亮的图形的宽高
            var se = this.getSouthEastBound();  //x  y   为图形的xy   
            var posX = mouseCurX;    //鼠标移动的x   可以移出图形外
            var xMulti, yMulti;
            switch (this._resizeCtrlIsDragging) {
                ...
                //     }
                //     this.setSizeByCorners({   //传入两个参数，左上角和右下角
                //         x: posX,
                //         y: mouseCurY
                //     }, {
                //         x: se.x,
...
crop.factory('cropHost', ['$document', '$q', 'cropAreaCircle', 'cropAreaSquare', 'cropAreaRectangle', 'cropEXIF', function ($document, $q, CropAreaCircle, CropAreaSquare, CropAreaRectangle, cropEXIF) {
    ...
    return function (elCanvas, opts, events) {
        ...
        var renderImageToDataURL = function (getResultImageSize) {
            ...
            temp_canvas.width = areaWidth;
            temp_canvas.height = areaHeight;
            if (image !== null) {
                ...
                if (resImgQuality !== null) {
                    retObj.dataURI = temp_canvas.toDataURL("image/png", resImgQuality);
                } else {
                    retObj.dataURI = temp_canvas.toDataURL("image/png");
                }
            }
            return retObj;
        };
...
(function(view) {
    ...
    if (HTMLCanvasElement && !canvas_proto.toBlob) {
        canvas_proto.toBlob = function(callback, type /*, ...args*/ ) {
            ...
            if (this.msToBlob && /^\s*image\/jpeg\s*(?:$|;)/i.test(type)) {
